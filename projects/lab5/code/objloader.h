...
namespace algorithm
{
	// vec3 Multiplication Opertor Overload
...
private:
	...
	void GenVerticesFromRawOBJ(std::vector<Vertex>& oVerts,
		...
		const std::vector<vec3>& iTCoords,
		...
		std::string icurline)
	{
		...
		for (int i = 0; i < int(sface.size()); i++)
		{
			...
			case 1: // P
			{
				vVert.pos = algorithm::getElement(iPositions, svert[0]);
				vVert.uv = vec3(0, 0);
				noNormal = true;
			...
			{
				vVert.pos = algorithm::getElement(iPositions, svert[0]);
				vVert.uv = algorithm::getElement(iTCoords, svert[1]);
				noNormal = true;
				oVerts.push_back(vVert);
				break;
			}
		...
		if (noNormal)
		{
			vec3 A = oVerts[0].pos - oVerts[1].pos;
			vec3 B = oVerts[2].pos - oVerts[1].pos;
		...
		const std::vector<Vertex>& iVerts)
	{
		...
		while (true)
		{
			...
			for (int i = 0; i < int(tVerts.size()); i++)
			{
				...
				float angle = math::AngleBetweenV3(pPrev.pos - pCur.pos, pNext.pos - pCur.pos) * (180 / 3.14159265359);
				...
				for (int j = 0; j < int(iVerts.size()); j++)
				{
					if (algorithm::inTriangle(iVerts[j].pos, pPrev.pos, pCur.pos, pNext.pos)
